...
export const getPublicCourses = async (req, res) => {
  try {
    ...
    // Only apply courseType filter when explicitly specified
    // For "all" (default), return all courses without filtering by courseType
    if (home === "true") {
    ...
    // For type === "all" or default, filter remains empty {} to return all courses

let courses = await Course.find(filter).sort({ createdAt: -1 }).lean();

    if (home === "true" && limit) {
      courses = courses.slice(0, Number(limit || 8));
    } else if (limit) {
...
export const getCourses = async (req, res) => {
  try {
    ...
    const mapped = courses.map((e) => ({
      ...e,
      image: makeImageAbsolute(e.image || "", req),
    }));
    return res.json({
      success: true,
      courses: mapped,
  ...
  try {
    ...
    if (!userId) {
      return res.status(401).json({
        success: false,
        message: "Aythentication ruquired",
      });
    }

    const { courseId } = req.params;
    const { rating: rawRating, commnet = "" } = req.body;
...
export const getMyRating = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      myRatings: my ? { rating: my.rating, comment: my.comment } : null,
    });
  } catch (err) {
